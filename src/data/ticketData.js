...
function sectionPrice(section) {
  if (section.id === '538') return 935;
  if (section.id === '531') return 959;
  if (section.tier === 'standing') return section.id.startsWith('FRONT') ? 3674 : 1250;
  if (section.tier === 'vip') return 8993;
  if (section.tier === 'club') return 2030;
  if (section.tier === 'lower') return section.id >= '146' || section.id <= '105' ? 3904 : 2261;
  return 1093;
}

function defaultAvailability(section) {
  if (section.tier === 'standing') return Math.max(20, Math.round(section.capacity * 0.006));
  if (section.tier === 'vip') return 12;
  if (section.tier === 'club') return 6;
  return 8;
}

export const stadiumSections = venueSections.map((section) => {
  const available = sectionAvailability[section.id] ?? defaultAvailability(section);
  return {
    ...section,
    sold: section.capacity - available,
    available,
    price: sectionPrice(section),
  };
});

export const totalVenueSold = stadiumSections.reduce((sum, section) => sum + section.sold, 0);
export const totalVenueRemaining = venueCapacity - totalVenueSold;
